    ...
    if (settings) {
        ...
        if (format == Totp::StorageFormat::LEGACY && m_ui->radioCustom->isChecked()) {
            auto answer = MessageBox::question(
                this,
                "Upgrade TOTP Settings",
                "You are currently using legacy TOTP settings which do not fully support custom settings.\n"
                "Would you like to upgrade to the latest format?",
